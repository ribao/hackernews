...
public class BestStoriesController : ControllerBase
{
    ...
    public BestStoriesController(ILogger<BestStoriesController> logger, IMemoryCache cache, Func<HttpClient> httpClientFactory)
    {
        _logger = logger;
        _cache = cache;
    ...
    {
        var storyList = await _cache.GetOrCreateAsync(BestStoriesResponseKey, async cacheEntry => await GetBestStories(cacheEntry));

        if (storyList == null)
            return Array.Empty<OutputStoryDetails>();
        if (limit is > 0)
        {
            storyList = storyList.Take(limit.Value);
        }

        var tasks = storyList.Select(id => _cache.GetOrCreateAsync<OutputStoryDetails>(id, async cacheEntry => await GetStoryDetails(id, cacheEntry)));
    ...
    private async Task<OutputStoryDetails> GetStoryDetails(int id, ICacheEntry cacheEntry)
    {
        try
        {
            ...
            return storyDetails == null ? null : new OutputStoryDetails(storyDetails);
        }
        catch (Exception e)
        {
